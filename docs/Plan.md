...
__*Примечание:*__ считается, что в поле настроена "маска", которая пропустит только первые 16 вводимых символов. Ввести больше не получится.
...
__*Примечание:*__ считается, что в поле настроена "маска", которая игнорирует любое количество введенных пробелов. Поле заполняется только числовыми символами.
...
__*Примечание:*__ считается, что в поле настроена "маска", которая пропустит только первые 3 вводимых символа. Ввести больше не получится.
...
1. **IntelliJ IDEA Community Edition 2023.2.2** – гибкая и удобная среда разработки с поддержкой Java и всех актуальных технологий и фреймворков.
2. **Postman** – это программа, которая предоставляет вам все необходимые инструменты для тестирования **API**: возможность посылать запросы, писать документацию к **API**, запускать автотесты, составлять иерархию тестов, проводить версионирование и так далее.
3. **Java 11** – язык программирования (в нашем случае для написания авто-тестов). Имеет набор готового ПО для разработки и запуска приложений.
4. **Gradle** – система сборки проекта, с помощью которой возможно управление подключёнными зависимостями и генерация отчётов о тестировании.
5. **JUnit 5** – фреймворк для запуска авто-тестов. Предоставляет множество новых функций и возможностей по сравнению с **JUnit4** и **TestNG**.
6. **Docker** – это программное обеспечение для разработки, доставки и запуска контейнерных приложений. Он позволяет создавать контейнеры, автоматизировать их запуск и развертывание, управляет жизненным циклом. С помощью **Docker** можно запускать множество контейнеров на одной хост-машине.
7. **Selenide** – фреймворк для автоматизированного тестирования веб-страниц и веб-приложений, который позволяет быстро и просто его использовать при написании тестов.
8. **Lombok** – основанная на аннотациях библиотека **Java**, позволяющая сократить шаблонный код, уменьшает трудозатраты и время на разработку и обеспечивает некоторую дополнительную функциональность.
9. **Faker** – библиотека для генерации тестовых данных.
10. **Rest Assured** – библиотека, позволяет пользователям обрабатывать HTTP-запросы, которые можно записать в достаточно читабельном формате.
11. **Allure** – фреймворк для создания отчетов о тестировании, более подробных чем в **Gradle**.
12. **Git** и **GitHub** – для ведения репозитория по проектам. **Git** очень распространенная система контроля версий, поэтому достаточно хорошо взаимодействует с различными ОС. **GitHub** специализированный веб-сервис с удобным интерфейсом, интегрирован с **Git**.
13. **AppVeyor Continuous Integration** – веб-сервис, предназначенный для сборки и тестирования программного обеспечения расположенного на **GitHub**, использующий виртуальные машины **Microsoft** **Windows** и **Ubuntu**. Бесплатный и удобный сервис, который может осуществлять сборку как под управлением **Linux**, так и под **Windows**, а если необходимо то и под несколькими сразу.

***

## 3. Перечень и описание возможных рисков при автоматизации.
- Сложности в настройке **Docker**. Необходимо проверить поддержку 2-х СУБД: **MySQL** и **PostgreSQL** одновременно, а также настроить работу симулятора банковских сервисов.
- Отсутствие постановки задач к тестируемому функционалу приложения.
- Отсутствие документаци по API.
- Неуникальные локаторы, которые затрудняют поиск элементов.
- Поскольку приложение состоит из нескольких страниц, их структура может оказаться достаточно сложной, поэтому для написания *pageObject* потребуется больше времени.
- Утрата актуальности авто-тестов при изменениях структуры приложения.
...
| Написание и прогоны авто-тестов         | 32       |
...
| Написание авто-тестов и их отладка                                          | 27.11.2023          |
