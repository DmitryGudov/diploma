...
2. **Postman** – это программа, которая предоставляет вам все необходимые инструменты для тестирования **API**:
...
3. **Java 11** – язык программирования (в нашем случае для написания авто-тестов). Имеет набор готового ПО для
...
5. **JUnit 5** – фреймворк для запуска авто-тестов. Предоставляет множество новых функций и возможностей по сравнению с
   **JUnit4** и **TestNG**.
6. **Docker** – это программное обеспечение для разработки, доставки и запуска контейнерных приложений. Он позволяет
...
7. **Selenide** – фреймворк для автоматизированного тестирования веб-страниц и веб-приложений, который позволяет быстро
   и просто его использовать при написании тестов.
8. **Lombok** – основанная на аннотациях библиотека **Java**, позволяющая сократить шаблонный код, уменьшает
   трудозатраты и время на разработку и обеспечивает некоторую дополнительную функциональность.
9. **Faker** – библиотека для генерации тестовых данных.
10. **Rest Assured** – библиотека, позволяет пользователям обрабатывать HTTP-запросы, которые можно записать в
    достаточно читабельном формате.
11. **Allure** – фреймворк для создания отчетов о тестировании, более подробных чем в **Gradle**.
12. **Git** и **GitHub** – для ведения репозитория по проектам. **Git** очень распространенная система контроля версий,
...
13. **AppVeyor Continuous Integration** – веб-сервис, предназначенный для сборки и тестирования программного обеспечения
...
- Поскольку приложение состоит из нескольких страниц, их структура может оказаться достаточно сложной, поэтому для
  написания *pageObject* потребуется больше времени.
